...
${end.comment}` : end.comment;
  ...
  // tools/bundle-src/yaml-bundle.js
  ...
  function diagnosticFromYamlError(error, severity = "error") {
    const pos = Array.isArray(error && error.pos) && Number.isFinite(error.pos[0]) ? error.pos[0] : 0;
    return {
      from: Math.max(0, pos),
      to: Math.max(1, pos + 1),
      severity,
  ...
  function fromJsonText(source) {
    return stringify3(JSON.parse(source || ""));
  }
  window.EditorWorkbenchYaml = {
    parseYaml,
    parseYamlDocument,
    stringifyYaml: stringify3,
    lintYaml,
    toJsonText,
    fromJsonText
  };
