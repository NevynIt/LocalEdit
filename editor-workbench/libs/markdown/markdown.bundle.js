...
var EditorWorkbenchMarkdownBundle = (() => {
...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  // tools/bundle-src/markdown-bundle.js
  var sanitizeOptions = {
  ...
  async function renderDiagramFence(language, source) {
    if (isMermaidFence(language)) {
      ...
      const svg2 = await window.EditorWorkbenchMermaid.renderMermaidSvg(source);
      return `<div class="diagram diagram-mermaid">${svg2}</div>`;
    }
    if (isGraphvizFence(language)) {
      ...
      const svg2 = await window.EditorWorkbenchGraphviz.renderGraphvizSvg(source);
      return `<div class="diagram diagram-graphviz">${svg2}</div>`;
    }
    return void 0;
  }
  ...
  async function renderMarkdown(text2) {
    const markdownWithDiagrams = await replaceDiagramFences(text2 || "");
    return sanitizeHtml(g.parse(markdownWithDiagrams));
  }
...
/*! For license information please see markdown.bundle.js.LEGAL.txt */
